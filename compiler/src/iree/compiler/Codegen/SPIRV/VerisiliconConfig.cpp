...
//===- VersiliconConfig.h - VERSILICON CodeGen Configurations ---------------------===//
//
// This file contains CodeGen configurations for VERSILICON GPUs & NPUs.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "iree-spirv-verisilicon-config"
...
static LogicalResult setOpConfig(const spirv::TargetEnv &targetEnv,
  ...
  if (!targetEnv.allows(spirv::Capability::CooperativeMatrixVSI) ||
      !targetEnv.allows(spirv::Extension::SPV_VSI_cooperative_matrix)) {
    return success();
  }
